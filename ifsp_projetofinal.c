...
int main(){
  ...
  do
  {
    system("cls");//Quando for necessário voltar, essa função limpará a tela
    printf("Vamos criar um vetor.\n");
    printf("Gostaria de digitar os valores? (Digite 1 para Sim ou 0 para Nao)\n");//Conforme a opção do usuário, cairá em um "for" diferente
    scanf("%d",&alfa);
    if (alfa==1){
      ...
      if(soma>0){//E testamos se é positivo ou não
        printf("A soma eh positiva\n");
      }
      else{	
        printf("A soma eh negativa\n");
      }
      printf("A multiplicação dos numeros eh %d\n",mult);	
      if (mult>=0){
        printf("A multiplicacao eh positiva\n");
      }
      else{
        printf("A multiplicação eh negativa\n");
      }
      system("pause");
      gama=1;
    }
      ...
      for (i=0;i<5;i++){
        vet[i]=rand()%100-50;
        printf("O valor de vet[%d]eh\t%04d",i+1,vet[i]);
        soma+=vet[i];
        mult*=vet[i];
        printf("\n");
      }
      ...
      if (mult>=0){
        ...
        printf("A multiplicacao eh negativa\n");
      }
      system("pause");
      gama=1;
    }
    else{
      gama=0;//Se gama for zero, o "while" entenderá como falso e repetirá o loop. E gama somente vale 0 quando alfa não for 1 ou 0. 
